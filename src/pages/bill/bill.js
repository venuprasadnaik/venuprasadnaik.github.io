...
const referenceID = `pay_${randomstring.generate({ length: 14, charset: alphabet })}`;
...
async function savePdf(htmlFile, outputFile) {
    ...
    await page.pdf({ path: outputFile + ".pdf", ...pdfOptions, width: "22cm", height:"29.71cm", margin:[0,0,0,0]});
    await browser.close();
}
...
for (const name of names) {
    ...
    savePdf(fileContent, name + "_" + currentDate.monthLong + "_" + currentDate.year);
}
